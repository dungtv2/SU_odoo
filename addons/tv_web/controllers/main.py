...
class Binary(http.Controller):
    ...
    def my_upload_attachment(self, callback, model, id, ufile):
        ...
        _model = request.env['ir.attachment']
        ...
        for ufile in files:
            ...
            if re.search(".mp4|.avi", file_name):
                store_path = tools.config['video_store_path']
                url_meta = tools.config['url_video']
            else:
                store_path = tools.config['store_path']
                url_meta = tools.config['url_img']
            ...
            try:
                ...
                attachment = _model.create({
                    'name': file_name,
                    'type': 'url',
                    'url': "%s/%s/%s" % (url_meta, folder_flow_date, file_name),
                    'res_model': model,
                    'res_id': int(1),
                    'is_store': True,
                })
